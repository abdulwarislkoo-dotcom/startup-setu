...
export default function Navbar({ 
  ...
  const loadNotifications = () => {
    ...
    fetch(`/api/notifications?email=${encodeURIComponent(currentUser.email)}`)
      .then(response => {
        if (!response.ok) throw new Error(`Notification request failed with status ${response.status}`);
          ...
          <div className="flex items-center gap-3">
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border border-slate-200 p-4 z-50">
                    <div className="flex items-center justify-between pb-2 border-b border-slate-100 mb-3">
                      ...
                      <span className="text-[11px] bg-blue-100 text-blue-800 font-bold px-2 py-0.5 rounded-full">{notifications.length} New</span>
                    </div>
                    <div className="space-y-3">
                      {notifications.length === 0 && <p className="text-xs text-slate-500">No new notifications.</p>}
                      {notifications.slice(0, 4).map(notification => (
                       <div key={notification.id} className="text-xs p-2 bg-blue-50 rounded-lg border border-blue-100">
                         <p className="font-semibold text-blue-900">{notification.title}</p>
                         <p className="text-slate-600 mt-0.5">{notification.message}</p>
                         <span className="text-[10px] text-slate-400 mt-1 block">{new Date(notification.createdAt).toLocaleString()}</span>
                       </div>
                      ))}
            ...
            {currentUser ? (
              <div className="flex items-center gap-3 pl-2 border-l border-slate-200">
                <div
                  className="w-8 h-8 rounded-full ring-2 ring-blue-600/20 bg-blue-100 text-blue-800 flex items-center justify-center text-xs font-bold"
