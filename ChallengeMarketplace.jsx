...
export default function ChallengeMarketplace({ 
  ...
  const [matchByChallenge, setMatchByChallenge] = useState({});

  React.useEffect(() => {
    if (currentUser?.role !== 'Startup' || !currentUser.startupId) return;
    const loadMatches = async () => {
      const results = await Promise.all((challenges || []).map(async challenge => {
        const response = await fetch('/api/ai/match-startups', {
          method: 'POST',
        ...
        return [challenge.id, data.matches.find(match => match.startupId === currentUser.startupId)];
      }));
      setMatchByChallenge(Object.fromEntries(results.filter(Boolean)));
    };
    loadMatches();
        ...
        {filteredChallenges.map((c) => (
          ...
          >
            ...
            <div className="space-y-4 pt-3 border-t border-slate-100">
              ...
              {currentUser?.role === 'Startup' && matchByChallenge[c.id] && (
                <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-900">
                  <strong>AI suitability:</strong> {matchByChallenge[c.id].matchScore}% — {matchByChallenge[c.id].badge}
                </div>
              )}
