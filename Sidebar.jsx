...
export default function Sidebar({ activeRole, activeTab, onSelectTab, collapsed, onToggle }) {
  ...
  return (
    <aside className={`${collapsed ? 'w-20' : 'w-64'} bg-slate-900 text-slate-300 min-h-[calc(100vh-4rem)] flex flex-col justify-between p-4 shrink-0 shadow-lg select-none transition-all duration-200`}>
      <div className="space-y-6">
        <button
          ...
          className={`w-full flex items-center ${collapsed ? 'justify-center' : 'justify-between'} gap-2 px-3 py-2 rounded-lg text-xs font-semibold text-slate-300 bg-slate-800 hover:bg-slate-700 hover:text-white border border-slate-700 transition-colors`}
        >
          {!collapsed && <span>{collapsed ? 'Expand' : 'Minimize'} menu</span>}
          {collapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}
        </button>
        
        {/* Role Badge Indicator */}
        <div className={`bg-slate-800/90 rounded-xl p-3 border border-slate-700/60 ${collapsed ? 'flex justify-center' : ''}`}>
          {collapsed ? (
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" title={activeRole}></span>
          ) : (
            <>
