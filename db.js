...
class Store {
  ...
  save(dataToSave) {
    const nextData = dataToSave || this.data;
    fs.writeFileSync(DATA_FILE, JSON.stringify(nextData, null, 2));
    if (this.mongoStore) {
      this.queueMongoSave(nextData);
    }
  }
  ...
  getMongoUri() {
    ...
    const host = process.env.MONGODB_HOSTS?.trim()
      || 'ac-efmxtqr-shard-00-00.uk87ypu.mongodb.net:27017,ac-efmxtqr-shard-00-01.uk87ypu.mongodb.net:27017,ac-efmxtqr-shard-00-02.uk87ypu.mongodb.net:27017';
    const username = encodeURIComponent(process.env.MONGODB_USERNAME || '');
    const password = encodeURIComponent(process.env.MONGODB_PASSWORD || '');
    if (!username || !password || !host) {
      throw new Error('MONGODB_URI must be a MongoDB URI or a host with MongoDB credentials.');
    }
    const replicaSet = process.env.MONGODB_REPLICA_SET || 'atlas-uxrb21-shard-0';
    const protocol = host.includes(',') ? 'mongodb' : 'mongodb+srv';
    return `${protocol}://${username}:${password}@${host}/?tls=true&authSource=admin&retryWrites=true&w=majority&replicaSet=${encodeURIComponent(replicaSet)}`;
  }
  ...
  async connect() {
    ...
    const database = client.db(process.env.MONGODB_DATABASE || 'startupsetu');
    const collection = database.collection('app_store');
    const documents = await collection.find({}).toArray();

    if (documents.length === 0) {
      await collection.insertMany(
        Object.entries(this.data).map(([name, items]) => ({ _id: name, items }))
      );
    } else {
      for (const document of documents) {
        if (Array.isArray(document.items)) {
          this.data[document._id] = document.items;
        }
      }
      this.save(this.data);
    }
    ...
    this.mongoStore = collection;
    console.log(`MongoDB connected: ${database.databaseName}.app_store`);
  }

  queueMongoSave(data) {
    const collection = this.mongoStore;
    this.writeQueue = this.writeQueue.then(async () => {
      await Promise.all(
        Object.entries(data).map(([name, items]) =>
          collection.replaceOne({ _id: name }, { _id: name, items }, { upsert: true })
        )
      );
    }).catch((error) => {
